import { AddEditJobTitleComponent } from './components/add-edit-job-title/add-edit-job-title.component';
...
export class JobTitlesComponent implements OnInit {
  ...
  jobTitlesList$!: Observable<any>;
  jobTitlesCount: number = 0;
  tableHeaders: any = [];
  ...
  getAllJobTitles(): any {
    ...
    //   .pipe(
    //     map((res: any) => {
    //       this.jobTitlesCount = res?.total;
    //       this.pagesCount = Math.ceil(this.jobTitlesCount / this.perPage);
    //       let arr: any = [];
  ...
  getUsers(): void {
    let arr: any = this.jobTitlesList$
  ...
  addOrEditItem(item?: any, type?: any): void {
    const ref = this.dialogService?.open(AddEditJobTitleComponent, {
      data: {
        item,
        type: type == 'edit' ? 'edit' : 'add'
      },
      header: type == 'edit' ? this.publicService?.translateTextFromJson('dashboard.departments.editDepartment') : this.publicService?.translateTextFromJson('dashboard.departments.addDepartment'),
      width: '50%',
