...
export class DepartmentsComponent implements OnInit {
  ...
  getAllDepartments(): any {
    ...
    //   .pipe(
    //     map((res: any) => {
    //       this.departmentsCount = res?.total;
    //       this.pagesCount = Math.ceil(this.departmentsCount / this.perPage);
    //       let arr: any = [];
  ...
  getUsers(): void {
    let arr: any = this.departmentsList$
    arr?.length == 0 ? this.getAllDepartments() : '';
  }
  ...
  addOrEditItem(item?: any, type?: any): void {
    const ref = this.dialogService?.open(AddEditDepartmentComponent, {
      data: {
        item,
